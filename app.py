...
@st.cache_data(show_spinner=False)
def load_json(content: bytes, name: str) -> pd.DataFrame:
    try:
...
with st.sidebar:
    ...
    page = st.radio(
        "Navigate",
        [" Upload & Summary", " Data Cleaning", " Visuals", " Export & Report"],
    ...
    with col1:
        if st.button("↩Undo", use_container_width=True):
            st.write("DEBUG - history length:", len(st.session_state.get("df_history", [])))
            if st.session_state.df_history:
                ...
                st.success("Undone! Data restored to previous state.")
                st.rerun()
            else:
...
if page == " Upload & Summary":
    ...
    uploaded = st.file_uploader(
        ...
        help="Supports CSV, Excel and Json files",
    )
    ...
    with st.expander(" Missing Values", expanded=True):
        ...
        if ms_nonzero.empty:
            ...
            ax.set_xlabel("Missing %", color="#000000")
            ...
            if st.button("Apply Missing Value Action"):
                ...
                try:
                    for c in sel_cols:
                        ...
                        elif action == "Fill with Previous":
                            df2[c] = df2[c].ffill()
                        elif action == "Fill with Next vaule":
                            df2[c] = df2[c].bfill()
